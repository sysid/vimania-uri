            ...
            try:
                ...
                if "t" in flags:
                    ...
                    if term in ["st", "st-256color"]:
                        term = "st"
                    if term in ["urxvt", "rxvt-unicode", "rxvt-unicode-256color"]:
                        term = "urxvt"
                    ...
                    # A temporary fix.
                    if term in ["tilda", "pantheon-terminal", "terminology", "termite"]:

                        target = command.split(";")[0].split("--")[1].strip()
                        app = command.split(";")[1].split("--")[0].strip()
                        cmd = [os.environ["TERMCMD"], cmdflag, "%s %s" % (app, target)]
                    elif term in ["guake"]:
                ...
                if "f" in flags or "t" in flags:
                    ...
                    with Popen(cmd, env=self.hook_environment(os.environ)) as process:
                        process.wait()
            finally:
...
def find_conf_path():
    ...
    if not os.path.isfile(conf_path):
        conf_path = os.path.normpath(
            os.path.join(os.path.dirname(__file__), "../config/rifle.conf")
        )
    if not os.path.isfile(conf_path):
        try:
...
def main():  # pylint: disable=too-many-locals
    ...
    if options.w is not None and not options.l:
        ...
        if options.l:
            ...
            result = rifle.execute(
                positional, number=number, label=label, flags=options.f
            )
            if result == ASK_COMMAND:
                # TODO: implement interactive asking for file type?
                print("Unknown file type: %s" % rifle.get_mimetype(positional[0]))
